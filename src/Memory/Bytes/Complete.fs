module NomFs.Memory.Bytes.Complete
...
let internal concat (a: ReadOnlyMemory<'a>) (b: ReadOnlyMemory<'a>) =
...
let takeWhile predicate =
    let inner (input: ReadOnlyMemory<'a>) =
        let res = NomFs.Memory.ReadOnlyMemory.takeWhile predicate input
        Ok (input.Slice(res.Length), res)
    inner

let takeWhileMN m n f =
    let inner (input: ReadOnlyMemory<'a>) =
        let res = input |> NomFs.Memory.ReadOnlyMemory.takeWhile f
